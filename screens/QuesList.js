...
userID:this.props.navigation.state.params.userID
    ...
    .then((response) => response.json())
    .then((responseJson) => {
    console.log('My Data List' + responseJson)
     this.setState({
  ...
  render() {
    ...
    const lapsList = this.state.answerList.map((data,index) => {
      ...
      <View key={index+1} style={[styles.block2, data.id == this.state.rightans  ? { backgroundColor:'green'}: data.id == this.state.myanswer ? { backgroundColor:'red'}:{}]}>
        <Text style={[styles.block2, data.id == this.state.rightans  ? { color:'white'}: data.id == this.state.myanswer ? { color:'white'}:{}]}>{data.id} ) {data.value}  {this.state.myanswer}</Text>
...
<Modal isVisible={this.state.modalVisible}  transparent={true} style={{marginTop:100,narginBottom:50}}>
...
/></TouchableOpacity>
    ...
    <View style={{marginTop:20,paddingTop:10, flex:1}}>
    {lapsList}
